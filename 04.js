...
const markBoards = (number, boards) =>
  boards.map(map(map(n => (n === number ? true : n))));

const drawUntilWinner = ({ numbers, boards }) =>
  until(
...
const calculateScore = converge(multiply, [
  pipe(get("winner"), flatten, reject(isTrue), sum),
  get("number"),
]);

// -- PART A----------------------------------------------------------------
readFile("./04.sample").then(
  pipe(parseInput, drawUntilWinner, calculateScore, result =>
    expect(result).toEqual(4512, "Day 04, Part A: Sample ok")
  )
);

readFile("./04.input").then(
  pipe(parseInput, drawUntilWinner, calculateScore, result =>
    console.log("Day 04, Part A: ", result)
  )
);
...
readFile("./04.sample").then(
  pipe(
    parseInput,
    until(round => round.boards.length === 1, drawUntilWinner),
    drawUntilWinner,
    calculateScore,
