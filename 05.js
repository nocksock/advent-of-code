import { map, range, repeat, zip, filter, flatten, join, transpose, tap, pipe, prop  } from "ramda";
import { expect, readFile} from "./helpers.js";

console.log("\n\n-- DAY 05--")
const parseInput = s => s
  .trim()
  .split("\n")
  .map(l => l
    .split(",")
...
const arrayPad = (a, l, f) => a.length > l ? a : a.concat(Array(l).fill(f)).slice(0, l);
...
const drawMap = lines => lines.reduce((map, line) => {
  const [from, to] = line;
...
readFile("./05.sample")
  .then(pipe(
    ...
    drawMap,
    countOverlaps(2),
    result => expect(result).toEqual(5, "Day 05, Part A: Sample ok")
  ))
...
readFile("./05.sample")
  .then(pipe(
    ...
    tap(render),
    countOverlaps(2),
    result => expect(result).toEqual(12, "Day 05, Part B: Sample ok")
  ))
...
readFile("./05.input")
  .then(pipe(
    ...
    drawMap,
    countOverlaps(2),
    result => console.log("Day 05, Part A Result: ", result)
  ))
...
readFile("./05.input")
  .then(pipe(
    ...
    drawMap,
    countOverlaps(2),
    result => console.log("Day 05, Part B Result: ", result)
  ))
