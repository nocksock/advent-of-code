import { divide, multiply, nth, tryCatch } from "ramda"
...
const completeBrackets = pipe(
  ...
  reduce(
    (acc, cv) => 
      isClosing(cv) ? acc.slice(1) : [pair[cv], ...acc],
    []
  ),
  join("")
)
...
const getMiddle = converge(nth, [pipe(prop("length"), multiply(0.5), Math.floor), sort(subtract)])
...
aoc
  ...
  .then(
    pipe(
      ...
      tap(pipe(
        partB,
        aoc.expectEqual(288957, "Part B, sample: ok")
      ))
    )
  )
...
expect(getMiddle([ 288957, 5566, 1480781, 995444, 294 ])).toEqual(288957, 'getmiddle')
